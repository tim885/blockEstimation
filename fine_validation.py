...
parser.add_argument('--dataset', metavar='DIR', default='/PATH_TO_DATASETS/UnLoc_Lab_Dataset/', help='path to dataset : ../UnLoc_Lab_Dataset/ or ../UnLoc_Field_Dataset/ or ../UnLoc_Adv_Dataset/')  # dataset dir argument
parser.add_argument('--model', default = '/DIRECTORY_PATH/fine_estimation/checkpoint_192.pth.tar',type=str, metavar='PATH', help='path to model')

def main():

   '''Here we fill the dict realPictData. Each realPict object corresponds to one block pose. realPict object is filled with the path of each picture from differents cameras, and with the value (X, Y, Rot) of the block w.r.t. the robot base. '''
   ...
   for index, row in csv.iterrows():
      ...
      #the pictures for this fine estimation step are the cropped one obtained with tool_detection
